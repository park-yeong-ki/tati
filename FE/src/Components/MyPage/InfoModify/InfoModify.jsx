...
export default function InfoModify() {
  ...
  const handleSendNickName = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_URL}/member/nickname-check`,
        {
  ...
  const handleNickNameupdata = () => {
    ...
    formData.append(
      "putMemberReqDto",
      new Blob([JSON.stringify(putMemberReqDto)], {
    ...
    // console.log(process.env.REACT_APP_URL)
    axios
      .put(`${process.env.REACT_APP_URL}/member/mypage/modifyNickName`, formData, {
        headers: {
  ...
  const handleSendPassword = () => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_URL}/member/mypage/modifyPassword`,
        {
          memberId: user.memberId,
          password,
        },
  ...
  const confirmWithdrawal = () => {
    axios
      .delete(
        `${process.env.REACT_APP_URL}/member/mypage/remove/${parseJwt.sub}`,
        {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("accessToken"),
            RefreshToken: localStorage.getItem("refreshtoken"),
    ...
    <div className={style.container}>
      <div className={style.info_box}>
        <div className={style.btns}>
          ...
          <button onClick={handleWithdrawal} className={style.Withdrawal_btn}>
            회원탈퇴
          </button>
        </div>
            ...
            <>
              <div className={style.user_profile}>
                ...
                <p className={style.InfoModify_profile}>
                  ...
                  <label
                    htmlFor="profile"
                    className={`${style.customFileInput} ${style.centeredText}`}
                  >
                    이미지 업로드
                    <input
                  ...
                  {imageDataURL && (
                    <img src={imageDataURL} alt="Uploaded" className={style.file_img} />
                  )}
                </p>
                <p>
                  닉네임
                  ...
                  />
                  {/* <button
                    className={style.InfoModify_nickname_check_btn}
              ...
              <button
                className={`${style.InfoModify_change} ${style.profile_btn}`}
                onClick={handleNickNameupdata}
              >
                완료
              </button>
            </>
          ...
          <>
            <div className={style.password_change}>
              ...
              <p>
                비밀번호확인
                <input
                  className={style.InfoModify_password2}
                  name="password2"
                  value={password2}
