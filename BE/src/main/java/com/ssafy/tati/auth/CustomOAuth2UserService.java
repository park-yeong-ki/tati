...
//            memberAttribute.put("exist", false);
            ...
            //회원 가입
            Member member = memberMapper.oAuth2AttributeToMember(oAuth2Attribute);
            Member createdMember = memberRepository.save(member);

            memberAttribute.put("memberId", createdMember.getMemberId());
            memberAttribute.put("memberName", createdMember.getMemberName());

            // 회원의 권한과 회원속성, 속성이름을 이용해 DefaultOAuth2User 객체를 생성해 반환한다.
            return new DefaultOAuth2User(
                    Collections.singleton(new SimpleGrantedAuthority("ROLE_".concat(createdMember.getRole()))),
                    memberAttribute, "email");
        }

        // 회원이 존재할경우, memberAttribute의 exist 값을 true로 넣어준다.
//        memberAttribute.put("exist", true);
        memberAttribute.put("memberId", findMember.get().getMemberId());
        memberAttribute.put("memberName", findMember.get().getMemberName());
